...
| 2 | Severity products over NC (MTBS / SE FireMap / MOSEV) | have | already in your docx draft; deck slides 101, 123 |
| 3 | Recall vs reference perimeters, panels a–d | have | deck slides 78, 111; `validation.summarize_validation`. Panel order per comment 13: a NIFC_IFPH, b GEOMAC, c TNC Coastal Plain, d TNC Sandhills |
| 4 | Matching: geography · covariate space · love plot | have | deck slides 17, 65, 66; `modeling/plotting.py` (`plot_balance`) |
| 5 | Event study + aggregated ATT | **regen** | deck slide 43 — **error bars are broken there** (NaN bootstrap replicates). Re-run with `randomization_inference()` |
| 6 | Odds ratios, forest plot, both covariate specs | new | `modeling/models.py` odds-ratio table → forest plot. Needs TODO A2 |
| 7 | Burn rate vs covariate, 8 equal-count bins | have | deck slides 20–34 (FireCCIS311 300 m; MCD64A1 300 m and 500 m). Pick one combination for the main text |

**Cut:** annual burned area over NC peat by product. It shows only that the
products disagree; with no ground truth in the panel it cannot say which is
right, so it supports none of Cat's four asks. Figure 3 (recall against
reference perimeters) is what adjudicates between products.
...
| 3 | Recall table underlying Fig 3 | have | `validation.summarize_validation` output. Redundant with Fig 3 — pick one |
...
- `notebooks/run_fire_comparison.ipynb` — product intercomparison (Fig 2).
- `notebooks/validate_against_reference.ipynb` — reference validation (Fig 3,
  Tab 3).
- `notebooks/peat_restoration_exploration.ipynb` — site exploration (Tab 1).
- `notebooks/modeling.ipynb` — matching, DiD, logistic (Figs 4, 5, 6, 7;
  Tabs 4, 5, 6).
